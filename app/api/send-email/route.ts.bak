...
export async function POST(request: NextRequest) {
  try {
    console.log('ðŸ“§ Send-email API route called');
    ...
    if (!to || !subject || !html) {
      console.error('ðŸ”´ Missing required fields:', { to: !!to, subject: !!subject, html: !!html });
    ...
    if (error) {
      console.error('âŒ Resend API error:', error);
      return NextResponse.json({ 
        error: error.message,
        details: error 
      }, { status: 500 });
